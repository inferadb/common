...
//! use inferadb_common_storage::{OrganizationSlug, StorageError};
//!
...
//! ) -> Result<(), StorageError> {
//!     store.create_key(organization, key).await
//! }
//! ```
...
use crate::{
    ...
    types::OrganizationSlug,
};

/// Storage key prefix for signing keys within an organization.
...
///
/// All operations take an `organization` parameter. Keys are stored at
/// `signing-keys/{kid}` within each organization.
///
/// # Method Naming
///
...
pub trait PublicSigningKeyStore: Send + Sync {
    ...
    ///
    /// The key is stored at `signing-keys/{kid}` within the organization.
    /// If a key with the same `kid` already exists, this operation fails.
    ///
    /// # Arguments
    ///
    /// * `organization` — Organization to store the key in
    ...
    /// Lists all active public signing keys for an organization.
    ///
    ...
    async fn create_keys(
        ...
        for key in keys {
            results.push(self.create_key(organization, key).await);
        }
        results
    }
    ...
    async fn revoke_keys(
        ...
        for &(kid, reason) in keys {
            results.push(self.revoke_key(organization, kid, reason).await);
        }
        results
    }
    ...
    async fn rotate_key(
        ...
        self.create_key(organization, new_key).await?;

        // Revoke the old key — if this fails, roll back the new key
        if let Err(e) = self.revoke_key(organization, old_kid, Some("key rotation")).await {
            // Best-effort rollback: delete the newly created key
            let _rollback = self.delete_key(organization, &new_key.kid).await;
            return Err(e);
        }
...
///     
///     let retrieved = store.get_key(org, "test-key-1").await?;
...
pub struct MemorySigningKeyStore {
    /// Keys indexed by (organization, kid).
    keys: Arc<RwLock<HashMap<(OrganizationSlug, String), PublicSigningKey>>>,
}
...
impl MemorySigningKeyStore {
    ...
    fn make_key(organization: OrganizationSlug, kid: &str) -> (OrganizationSlug, String) {
        (organization, kid.to_string())
    }
}
...
impl PublicSigningKeyStore for MemorySigningKeyStore {
    ...
    async fn create_key(
        ...
        let map_key = Self::make_key(organization, &key.kid);
    ...
    async fn rotate_key(
        ...
        let old_map_key = Self::make_key(organization, old_kid);
        let new_map_key = Self::make_key(organization, &new_key.kid);
...
mod tests {
    ...
    async fn test_create_and_get_key() {
        ...
        // Create the key
        store.create_key(organization, &key).await.expect("create_key should succeed");

        // Retrieve it
        let retrieved = store.get_key(organization, "key-1").await.expect("get_key should succeed");
    ...
    async fn test_get_nonexistent_key() {
        ...
        let result = store.get_key(OrganizationSlug::from(100), "nonexistent").await;
    ...
    #[tokio::test]
    async fn test_same_kid_different_organizations() {
        let store = MemorySigningKeyStore::new();
        let key1 = make_test_key("shared-kid");
        let key2 = make_test_key("shared-kid");

        // Same kid in different organizations should work
        store.create_key(OrganizationSlug::from(100), &key1).await.expect("create in org 100");
        store.create_key(OrganizationSlug::from(200), &key2).await.expect("create in org 200");

        let r1 =
            store.get_key(OrganizationSlug::from(100), "shared-kid").await.expect("get ns 100");
        let r2 =
            store.get_key(OrganizationSlug::from(200), "shared-kid").await.expect("get ns 200");
    ...
    async fn test_list_active_keys() {
        ...
        store.create_key(organization, &active_key).await.expect("create active");
        store.create_key(organization, &inactive_key).await.expect("create inactive");
        store.create_key(organization, &future_key).await.expect("create future");
        store.create_key(organization, &expired_key).await.expect("create expired");

        // Only the active key should be listed
        let active_keys = store.list_active_keys(organization).await.expect("list_active_keys");
    ...
    #[tokio::test]
    async fn test_list_active_keys_empty_organization() {
        let store = MemorySigningKeyStore::new();

        let result = store.list_active_keys(OrganizationSlug::from(999)).await;
    ...
    async fn test_deactivate_key() {
        ...
        store.deactivate_key(organization, "to-deactivate").await.expect("deactivate");

        let retrieved = store.get_key(organization, "to-deactivate").await.expect("get");
    ...
    async fn test_deactivate_nonexistent_key() {
        ...
        let result = store.deactivate_key(OrganizationSlug::from(100), "nonexistent").await;
    ...
    async fn test_revoke_key() {
        ...
        store.revoke_key(organization, "to-revoke", Some("compromised")).await.expect("revoke");

        let retrieved = store.get_key(organization, "to-revoke").await.expect("get");
    ...
    async fn test_revoke_key_idempotent() {
        ...
        store.revoke_key(organization, "revoke-twice", None).await.expect("first revoke");

        // Get the timestamp from first revocation
        let first = store.get_key(organization, "revoke-twice").await.expect("get");
        let first_revoked_at = first.expect("exists").revoked_at;

        // Second revocation should succeed and not change timestamp
        store.revoke_key(organization, "revoke-twice", None).await.expect("second revoke");

        let second = store.get_key(organization, "revoke-twice").await.expect("get");
    ...
    async fn test_revoke_nonexistent_key() {
        ...
        let result = store.revoke_key(OrganizationSlug::from(100), "nonexistent", None).await;
    ...
    async fn test_activate_key() {
        ...
        store.create_key(organization, &key).await.expect("create");
        store.deactivate_key(organization, "to-reactivate").await.expect("deactivate");

        // Verify it's inactive
        let inactive = store.get_key(organization, "to-reactivate").await.expect("get");
        assert!(!inactive.expect("exists").active);

        // Reactivate
        store.activate_key(organization, "to-reactivate").await.expect("activate");

        let reactivated = store.get_key(organization, "to-reactivate").await.expect("get");
    ...
    async fn test_activate_revoked_key_fails() {
        ...
        store.create_key(organization, &key).await.expect("create");
        store.revoke_key(organization, "permanently-revoked", None).await.expect("revoke");

        let result = store.activate_key(organization, "permanently-revoked").await;
    ...
    async fn test_delete_key() {
        ...
        store.delete_key(organization, "to-delete").await.expect("delete");

        let result = store.get_key(organization, "to-delete").await.expect("get");
    ...
    async fn test_delete_nonexistent_key() {
        ...
        let result = store.delete_key(OrganizationSlug::from(100), "nonexistent").await;
    ...
    async fn test_activate_nonexistent_key_fails() {
        ...
        let result = store.activate_key(OrganizationSlug::from(100), "nonexistent").await;
    ...
    async fn test_clone_store_shares_state() {
        ...
        store.create_key(OrganizationSlug::from(100), &key).await.expect("create via original");

        let result =
            cloned.get_key(OrganizationSlug::from(100), "shared").await.expect("get via clone");
    ...
    async fn test_revoke_key_without_reason() {
        ...
        store.revoke_key(organization, "no-reason", None).await.expect("revoke");

        let retrieved = store.get_key(organization, "no-reason").await.expect("get");
        let retrieved = retrieved.expect("exists");
    ...
    async fn test_revoke_key_idempotent_preserves_reason() {
        ...
        store
            .revoke_key(organization, "idempotent-reason", Some("compromised"))
            .await
            .expect("first revoke");

        let first = store.get_key(organization, "idempotent-reason").await.expect("get");
        ...
        store
            .revoke_key(organization, "idempotent-reason", Some("different reason"))
            .await
    ...
    async fn test_create_keys_bulk() {
        ...
        let results = store.create_keys(organization, &keys).await;
        ...
        for i in 0..10 {
            let key = store.get_key(organization, &format!("bulk-{i}")).await.expect("get");
            assert!(key.is_some());
        }
    }
    ...
    async fn test_create_keys_bulk_with_duplicate() {
        ...
        store.create_key(organization, &existing).await.expect("pre-create");
    ...
    async fn test_create_keys_empty() {
        ...
        let results = store.create_keys(organization, &[]).await;
    ...
    async fn test_revoke_keys_bulk() {
        ...
        let results = store.revoke_keys(organization, &keys_to_revoke).await;
        ...
        let k0 = store.get_key(organization, "revoke-bulk-0").await.expect("get").expect("exists");
        assert!(k0.revoked_at.is_some());
        assert_eq!(k0.revocation_reason.as_deref(), Some("rotation"));

        let k3 = store.get_key(organization, "revoke-bulk-3").await.expect("get").expect("exists");
        assert!(k3.revoked_at.is_some());
        assert!(k3.revocation_reason.is_none());

        // Non-revoked key should still be active
        let k5 = store.get_key(organization, "revoke-bulk-5").await.expect("get").expect("exists");
        assert!(k5.active);
    ...
    async fn test_revoke_keys_with_missing() {
        ...
        let mut results = store.revoke_keys(organization, &keys_to_revoke).await;
    ...
    async fn test_rotate_key_success() {
        ...
        store.create_key(organization, &old_key).await.expect("create old");

        let new_key = make_test_key("new-key");
        store.rotate_key(organization, "old-key", &new_key).await.expect("rotate");

        // Old key should be revoked
        let old =
            store.get_key(organization, "old-key").await.expect("get old").expect("old exists");
        assert!(!old.active);
        ...
        let new =
            store.get_key(organization, "new-key").await.expect("get new").expect("new exists");
        assert!(new.active);
    ...
    async fn test_rotate_key_old_not_found() {
        ...
        let new_key = make_test_key("new-key");
        let result = store.rotate_key(organization, "nonexistent", &new_key).await;

        assert_storage_error!(result, NotFound);

        // New key should not have been created
        let check = store.get_key(organization, "new-key").await.expect("get");
    ...
    async fn test_rotate_key_new_already_exists() {
        ...
        store.create_key(organization, &old_key).await.expect("create old");
        store.create_key(organization, &conflicting_new).await.expect("create conflict");

        let new_key = make_test_key("conflicting");
        let result = store.rotate_key(organization, "old-key", &new_key).await;
    ...
    async fn test_rotate_key_atomicity_on_failure() {
        ...
        let result = store.rotate_key(organization, "missing-old", &new_key).await;

        assert!(result.is_err());

        // Neither old (doesn't exist) nor new key should exist
        let check = store.get_key(organization, "new-key-atomic").await.expect("get");
        assert!(check.is_none());
    }
}
